...
Resolve `<feature>` from, in order: an existing `_CONTEXT_<feature>.md` file named in the request, the session argument, or the current feature branch name (slugified, lowercase, filesystem-safe).
...
2. Run the `/grilling` interview, one question at a time.
3. After every answer, append the question and answer to `_INTERVIEW_<feature>.md` before asking the next question.
4. When a domain term resolves, update `_CONTEXT_<feature>.md` inline using the `/domain-modeling` skill. Never treat the context file as a transcript or scratch pad.
